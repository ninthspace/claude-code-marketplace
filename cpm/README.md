...
**Install `ralph-loop@ninthspace-ralph` 1.2.0 or later.** That is the version CPM's documented loop behaviour is written against, and the first with all three of the fork's changes. Each fixes a failure that is silent — nothing errors, so a run below this version looks like a run above it:

| The fork's change | Without it |
|---|---|
| **Fail-closed extraction** — the state file survives a transcript the hook cannot read | Upstream deletes it on a missing file, a `grep` for assistant records that matches nothing, or a `jq` parse error. The state file *is* the loop, so the run ends with exit 0, no promise and no state file — indistinguishable, afterwards, from a run that finished. Upstream's own reasoning about the no-text-block case applies to all three: the extracted output feeds exactly one decision, whether the promise was emitted, so failing to read it means *no promise this iteration*. The costs are asymmetric — continuing wrongly wastes one iteration, stopping wrongly ends an overnight run silently. |
| **The hook does not print the promise inside promise tags** | Every `<promise>` in a 24-iteration transcript is the hook's own per-iteration reminder, so a grep for the emission returns 24 hits from the hook and none from the model until the last one. You cannot tell a loop that finished from one that was told how to finish, and neither can a script watching the run. The fork also emits `RALPH_PROMISE_MATCHED` on a genuine match, printed nowhere else. |
| **`active: false` pauses the loop** | The field is written into every state file and read by nothing, so the obvious way to pause a run has no effect whatsoever and says nothing. The only working stop is deleting the file, which discards the prompt and the iteration count with it. The fork's pause leaves the file byte-for-byte untouched, so setting it back to `true` resumes at the same iteration. |

`/cpm:ralph` does **not** check a plugin name or a version — pre-flight runs `cpm/hooks/lib/ralph-hook-probe.sh` against whichever hook is registered and branches on what it actually does, because a hook can be installed, registered, and still delete the state file at the first iteration boundary. So all three plugins work and the loop is not gated on this; the version above is what you want installed, not what is enforced. **Enable only one.** They register the same hook at the same path, both fire on the same session, and one of them deleting the state file is enough to end the loop.
