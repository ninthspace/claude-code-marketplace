#!/bin/bash
# test-audit-skill.sh — Structural tests for the cpm:audit skill
#
...
SKILL_FILE="$REPO_ROOT/cpm/skills/audit/SKILL.md"
PLUGIN_MANIFEST="$REPO_ROOT/cpm/.claude-plugin/plugin.json"
...
test_start "SKILL.md exists at cpm/skills/audit/SKILL.md"
...
test_start "Frontmatter description references the /cpm:audit trigger"
if [ -f "$SKILL_FILE" ]; then
  FRONTMATTER=$(awk '/^---$/{c++; next} c==1' "$SKILL_FILE")
  assert_contains "$FRONTMATTER" "/cpm:audit"
else
  test_fail "SKILL.md missing — cannot inspect frontmatter"
fi
...
test_start "cpm/.claude-plugin/plugin.json keywords contain 'audit'"
if [ -f "$PLUGIN_MANIFEST" ]; then
