...
3. **Gate on disposition, not acknowledgement.** Present each relevant observation **verbatim, with its category** (rendered in the message body per the **Gate Presentation** convention), and require a disposition for *each one* via `AskUserQuestion` — never a single blanket confirmation:
   ...
   - **Obsolete** — no longer relevant; say why. This disposition is **durable**: beyond the per-run breadcrumb (step 4), it retires the lesson at its source (step 5) so it never resurfaces in any future run.
...
5. **Retire obsolete lessons at the source (durable).** For each observation given the **Obsolete** disposition, append a retirement marker to that observation's bullet in its **source retro file** per the shared **Retro Retirement** convention — `**Retired {YYYY-MM-DD}**: {reason}`, using the *why* from step 3 as the reason. The bullet stays in place; the marker makes the shared **Retro Awareness** selection skip it in all future runs. The step-4 breadcrumb records the decision for *this* run; this source marker makes it durable across runs. **Applied** and **Deferred** dispositions never write to the source retro — only **Obsolete** retires.
