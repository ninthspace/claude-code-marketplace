...
1. **Find the lessons.** Glob `docs/retros/[0-9]*-retro-*.md`. If none exist or the directory is absent, skip the gate silently and start the loop. Otherwise read the most recent retro file (highest numeric prefix). **Make the selection visible**: when you present the gate (step 3), name the retro you selected — its filename, its `**Source**:` field, and its `**Date**:` — so the choice of *which* retro is consumed is explicit and a mis-selection (e.g. a more recent ad-hoc retro shadowing the epic-relevant one) is never silent. Selection is by recency (highest prefix) only; surfacing it lets a human catch the rare case where newest ≠ most relevant.
