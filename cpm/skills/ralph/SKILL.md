...
**What spec mode costs.** Substituting both clauses gives an assembled spec-mode prompt of **4,407 characters** — the template's 3,188 less the 103-character opening and the 927-character completion clause it replaces, plus the two blocks above. AD3 estimated "roughly 3,400" for carrying both phases; the first assembly came in at 3,656, the liveness guards took it to 4,077, and spec 46's `[target]` clause added the remaining 330. Both overruns are recorded rather than corrected, and for the same reason: the guards were bought with a live run that burned its whole iteration cap generating epic docs, and the `[target]` clause is what stops an autonomous run self-assessing a constraint it cannot check. Each is a worse thing to lose than the characters it costs. The figures are stated here because the epic-mode figure a suite asserts is a *different* number, and a reader who checks only that one would conclude the two-phase prompt was free.

**These four figures have no test.** `**Length:**` above is extracted and compared against the template line by two suites; the numbers in this paragraph are prose, and an edit to the template silently falsifies all four. They were re-measured by hand when spec 46 changed the template — the 103 and 927 spans were confirmed unchanged rather than assumed — but a stated figure whose only guard is the diligence of the next editor is retro 24's lesson waiting to recur.
