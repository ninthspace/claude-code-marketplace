...
- `{log_clause}` — the iteration-log clause below, with `{session_id}` already substituted so the loop never has to resolve it. Always included, in both modes: it is what makes a stalled run visible, and a run is never known in advance not to be the one that stalls.
...
#### The iteration log, and the stall it makes visible

The template carries the clause below, interpolated as `{log_clause}`. It is one line appended per iteration to `docs/plans/.cpm-ralph-log-{session_id}.md`, and one comparison against the lines already there:
...
**What spec mode costs.** Substituting both clauses gives an assembled spec-mode prompt of **5,080 characters** — the template's 3,367 less the 103-character opening and the 1,094-character completion clause it replaces, plus the two blocks above.
