...
**Four codes, and the two that must not collapse.** In the completion clause `3` and `4` both mean *do not promise yet*, and answering them the same way would be the easy edit — but they resume at different phases, so the clause names them separately. `1` and `2` mean *stop*, and reading either as `4` would send a loop that cannot read its own inputs into generating epics against a spec it never parsed. It is written as four branches rather than "0 means done, anything else means continue" because a branch on a code the script never returns reads perfectly and never fires — so the codes named here must be the codes the script actually emits.
