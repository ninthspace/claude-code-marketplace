...
**What spec mode costs.** Substituting both clauses gives an assembled spec-mode prompt of **5,226 characters** — the template's 3,460 less the 103-character opening and the 1,199-character completion clause it replaces, plus the two blocks above.
...
```
Stop the loop: set active: false in .claude/ralph-loop.local.md, leaving the rest of the file
unchanged. If the same prompt arrives again with the same verdict, the plugin does not read
that field -- delete .claude/ralph-loop.local.md instead.
```

**Why the pause rather than the delete, first.** `active: false` is non-destructive on `ralph-loop@ninthspace-ralph` (Step 1c): the file keeps its prompt and its iteration count, so whoever finds the run can read what it was doing and resume it by flipping the field back. Deleting works on every plugin and discards both. The retry sentence is what makes the pair safe on a plugin that ignores the field — the loop notices it is still running and escalates, rather than pausing into a no-op forever.
