...
6. **Coverage matrix invalidation**: After applying changes to an epic doc, check for its companion coverage matrix — deriving the path from the epic path by replacing `-epic-` with `-coverage-` in the filename, the same rule `cpm:do` uses. If it exists and the cascade modified any story acceptance criteria in the epic doc, read the coverage matrix and clear the `✓` from the Verified column for rows whose "Story Criterion (verbatim)" text was changed by the cascade. Rows whose criteria were not modified retain their `✓` status. Use the Edit tool to replace `| ✓ |` with `| |` for affected rows. If no coverage matrix exists at the derived path, say so and name the path checked rather than skipping silently.

   **Why the derivation and not a number and a slug.** Substituting `-epic-` for `-coverage-` resolves both epic filename shapes without detecting which one it is: `docs/epics/15-epic-foo.md` → `docs/epics/15-coverage-foo.md`, and `docs/epics/28-01-epic-foo.md` → `docs/epics/28-01-coverage-foo.md`. Rebuilding the path from "the same number and slug" does not, because a two-part prefix read as a single number yields a filename that was never written. That miss is silent and its consequence is not: the step exists to strip verification marks from criteria a cascade has just rewritten, so a matrix it fails to find keeps asserting that changed criteria were verified. An epic written by `cpm:epics` always has a companion, which is why an absence is worth reporting rather than passing over.
