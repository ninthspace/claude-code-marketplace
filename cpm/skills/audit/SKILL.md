...
Run a structured audit of the current codebase, producing a numbered audit document at `docs/audits/{nn}-audit-{slug}.md` with concrete findings, citations, and prioritised recommendations. The skill orients on the codebase, sweeps nine dimensions of code health, and ends by offering to pipe findings into `/cpm:library`, `/cpm:spec`, or `/cpm:quick`.

## Input

The skill is invoked via `/cpm:audit`. Parse `$ARGUMENTS` as an optional **scope hint**:

1. If `$ARGUMENTS` is empty, the audit covers all nine dimensions evenly across the whole codebase.
2. If `$ARGUMENTS` contains a hint (e.g. `/cpm:audit auth` or `/cpm:audit src/billing`), record it as the declared scope. The hint shapes which areas of the codebase receive deeper attention but does **not** cause any of the nine dimensions to be skipped.
...
#### 1e. Passive cpm artifact read

If the project uses cpm, read `docs/specifications/`, `docs/epics/`, `docs/briefs/`, and `docs/architecture/` (whichever exist) and surface them to the user as context — list the files found and brief descriptions where useful. The user should know the audit has seen the existing planning material.

> **Non-negotiable**: cpm artifact contents must NOT be used to skip any of the nine dimensions, shortcut a finding, or otherwise bias the independent sweep. The audit's value is independent observation. Existing artifacts are read for context only — never to deduce that a dimension "is already covered" or that a finding "has already been planned for". Every dimension is swept on its own merits.
...
> **Non-negotiable**: recommendations must NOT use the phrases "rewrite", "replace entirely", "rebuild", "ground-up rewrite", or any equivalent language that implies wholesale module/file/system replacement. The audit's job is to surface fixable debt, not to license large rewrites. If a finding genuinely warrants a rewrite, that's a discussion for `/cpm:spec` and `/cpm:architect` — the audit document records the symptom, not the rewrite recommendation.
...
> **Options**:
> - "Send findings to `/cpm:library`" — wrap the audit as a library reference document so other cpm skills can pick it up via Library Check.
> - "Promote priorities to `/cpm:spec`" — invoke the spec skill on the audit document path so the Top 5 Priorities can become a structured spec.
> - "Run quick wins through `/cpm:quick`" — invoke the quick skill on the audit document path so the checklist can be processed item-by-item.
...
```
Skill(skill: "cpm:spec", args: "docs/audits/{nn}-audit-{slug}.md")
```

`/cpm:spec` already accepts file-path `$ARGUMENTS` — the audit document is the contract.
...
```
Skill(skill: "cpm:quick", args: "docs/audits/{nn}-audit-{slug}.md")
```

`/cpm:quick` will read the audit, classify the request, and walk the quick wins checklist as the change description.
...
- **Independence from cpm artifacts.** `docs/specifications/`, `docs/epics/`, `docs/briefs/`, `docs/architecture/` are read as passive context only. They must not bias or shortcut the independent sweep.
