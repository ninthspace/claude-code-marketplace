 ...
 * @returns {Set<string>}
 */
function idColumns(db, table) {
  const exempt = new Set(
    db.prepare(`PRAGMA foreign_key_list(${table})`).all().map((row) => row.from),
  );
 ...
 * @throws {ToolError}
 */
export function refuseBareUlids(db, table, values, where, exempt = idColumns) {
...
export const nothingExempt = () => new Set();
