...
import { ancestryOf } from './naming.js';
 ...
 */
export function loadDocument(db, id) {
  const document = db.prepare('SELECT * FROM document WHERE id = ?').get(id);

  if (!document) throw new Error(`no document with id '${id}'`);

  const rows = db.prepare('SELECT * FROM document ORDER BY id').all();
  const ancestry = ancestryOf(new Map(rows.map((row) => [row.id, row])), document)
    .map((row) => ({ ...row }));

  const requirements = collection(db, 'requirements', id).map((requirement) => ({
    ...requirement,
    criteria: collection(db, 'criteria', requirement.id).map((criterion) => ({
      ...criterion,
 ...
 */
export function detailOf(db, kind, documentId) {
  const table = DETAIL_TABLES[kind];

  if (!table) return undefined;

  const row = db.prepare(`SELECT * FROM ${table} WHERE document_id = ?`).get(documentId);
 ...
 * @returns {object[]}
 */
export function storiesOf(db, epicId) {
  return collection(db, 'stories', epicId).map((story) => ({
    ...story,
    tasks: collection(db, 'tasks', story.id),
    criteria: collection(db, 'storyCriteria', story.id).map((criterion) => ({
      ...criterion,
 ...
 */
export function coverageFor(db, epic, spec) {
 ...
 * @returns {object[]}
 */
export function observationsOf(db, retroId) {
 ...
 * @returns {string}
 */
export function taxonomyLabel(db, taxonomyId) {
  if (taxonomyId === null || taxonomyId === undefined) return '';

  const term = db.prepare('SELECT name, singular FROM taxonomy WHERE id = ?').get(taxonomyId);
 ...
 * @returns {string}
 */
export function agentLabel(db, name) {
  if (name === null || name === undefined) return '';

  const agent = db.prepare('SELECT display_name, role FROM agent WHERE name = ?').get(name);
 ...
 */
export function adrDetail(db, documentId) {
  const detail = db.prepare('SELECT * FROM adr WHERE document_id = ?').get(documentId);

  if (!detail) return undefined;

  return {
    ...detail,
    options: collection(db, 'options', documentId).map((option) => ({
      ...option,
