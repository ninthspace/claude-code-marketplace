 ...
 */
export function pathOf(db, document, ...ancestry) {
  const kind = db.prepare('SELECT dir FROM document_kind WHERE kind = ?').get(document.kind);
 ...
 * @returns {object[]}
 */
export function ancestryOf(byId, document) {
  const chain = [];
