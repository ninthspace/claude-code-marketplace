 ...
 * @returns {(string|null)[]}
 */
export function adrBlocks(db, adr, ancestry, ref, level = 3) {
    ...
    ...detail.options.flatMap((option) => [
      ...
      option.tradeoffs.length > 0
        ? table(['Axis', 'Assessment'],
          option.tradeoffs.map((tradeoff) => [tradeoff.axis, ref(tradeoff.assessment)]))
        : null,
    ]),
  ];
}
 ...
 * @returns {string}
 */
export function renderAdr(db, tree, ref) {
 ...
 * @returns {(string|null)[]}
 */
export function adrSection(db, tree, ref) {
