...
import { bullet, heading, paragraph, table } from '../text.js';
...
export const approaches = (row) =>
  (row.approaches.length > 0 ? ` ${row.approaches.map((tag) => `\`[${tag}]\``).join(' ')}` : '');
 ...
 */
function requirement(row, ref) {
  const criteria = row.criteria.map((criterion) => bullet(
 ...
 * @returns {string}
 */
export function renderSpec(db, tree, identifiers, where) {
