...
import { renderLibrary } from './templates/library.js';
import { renderProse } from './templates/prose.js';
import { renderQuick } from './templates/quick.js';
import { renderRetro } from './templates/retro.js';
 ...
 */
export function renderDocument(db, id, names = identifiers(db)) {
 ...
 */
export function project(db, { root = '.', write = true } = {}) {
  ...
  const documents = db.prepare('SELECT id FROM document ORDER BY id').all();
