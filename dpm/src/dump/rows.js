 ...
 * @param {string} table
 * @returns {{columns: string[], source: 'primary-key'|'unique-index'|'all-columns'}}
 */
export function orderingOf(db, table) {
  ...
  return { columns: info.map((column) => column.name), source: 'all-columns' };
}
 ...
 *
 * @param {import('node:sqlite').DatabaseSync} db
 * @param {import('./objects.js').SchemaObject[]} objects  From `dumpableObjects`.
 * @param {string[]} owners  Virtual table names, whose rows are derived and skipped.
 * @returns {{sql: string, counts: Record<string, number>, orderings: Record<string, string>,
 *   normalised: {table: string, column: string, value: string}[]}}
 */
export function dumpRows(db, objects, owners) {
