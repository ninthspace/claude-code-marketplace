...
import { dumpableObjects, maintainsVirtualTable } from './objects.js';
 ...
 * @returns {{sql: string, deferred: string, kept: string[],
 *   excluded: {name: string, reason: string}[], owners: string[]}}
 *   `sql` precedes the rows and `deferred` follows them. Both halves keep `sqlite_schema`
...
export function dumpSchema(db) {
  ...
  const statement = (object) => `${object.sql};\n`;
  const deferrable = (object) => object.type === 'trigger' && !maintainsVirtualTable(object, owners);
