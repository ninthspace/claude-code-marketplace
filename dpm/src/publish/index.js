 ...
 * @throws {import('../projection/index.js').ProjectionError} When any document cannot be rendered,
 *   raised before the first byte is written.
 */
export function publish(db, { root = '.', dryRun = false } = {}) {
...
export function describe(record) {
  ...
  const created = written.filter((path) => !rewritten.includes(path));
