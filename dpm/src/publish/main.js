 ...
 *
 * Separated from `bin/dpm-publish.js` for the reason `guard/main.js` is separated from
...
import { ProjectionError } from '../projection/naming.js';
import { describe, publish } from './index.js';
...
export function run({ root = '.', location = DATABASE, streams } = {}) {
  ...
  try {
    ...
    err(`dpm: publishing ${database} failed — ${error.message}\n`);
