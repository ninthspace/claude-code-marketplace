 ...
 */
export function numberScopes(db) {
  const partial = db
 ...
 * @returns {object[]}
 */
export function documents(db) {
  return db.prepare(`SELECT ${COLUMNS} FROM document ORDER BY id`).all();
}
 ...
 */
export function collisions(db, scopes = numberScopes(db), rows = documents(db)) {
  const found = [];
 ...
 */
function freeNumber(db, row, claimed) {
 ...
 */
export function assignNumbers(db, scopes, rows) {
 ...
 */
export function renumber(db, id) {
  const row = db.prepare(`SELECT ${COLUMNS} FROM document WHERE id = ?`).get(id);
