...
import { dumpableObjects } from '../dump/objects.js';
import { orderingOf } from '../dump/rows.js';
 ...
 * @returns {string}
 */
export function keyOf(row, columns) {
 ...
 */
function mergeSequence(base, ours, theirs, into) {
  ...
  const next = Math.max(ours.next_value, theirs.next_value);
...
export function threeWay(base, ours, theirs) {
  ...
  for (const table of tables) {
    ...
    const into = { key: o.key, columns: o.columns, inserts: [], updates: [], deletes: [] };
...
export function applyPlan(db, plan, {
  ...
  for (const [table, change] of writes ? plan : []) {
    ...
    for (const original of change.updates) {
      ...
      db.prepare(`UPDATE ${quote(table)} SET ${set} WHERE ${where}`).run(...values);
      counts.updated += 1;
    }
    ...
    for (const original of change.inserts) {
      ...
      db.prepare(`INSERT INTO ${quote(table)} (${names}) VALUES (${placeholders})`)
        .run(...change.columns.map((column) => row[column]));
      counts.inserted += 1;
    }
  }
