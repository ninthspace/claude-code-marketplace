...
import { assignNumbers, collisions, documents, numberScopes } from './numbers.js';
import { applyPlan, MergeError, SEQUENCE_TABLE, snapshot, threeWay } from './rows.js';
 ...
 */
function prospective(db, change) {
  const rows = new Map(documents(db).map((row) => [row.id, row]));

  for (const row of [...change.updates, ...change.inserts]) {
    rows.set(row.id, {
      id: row.id,
      kind: row.kind,
...
function load(side, sql) {
  ...
  try {
    ...
    throw new MergeError(`the ${side} side of the merge does not restore — ${error.message}`);
  }
...
export function merge({ base, ours, theirs }) {
  ...
  try {
    ...
    try {
      ...
      const written = applyPlan(dbOurs, plan, {
        ...
        substitute: (table, row) => {
          if (table !== 'document' || !moves.has(row.id)) return row;
    ...
    try {
      ...
      throw new MergeError(`the merged dump does not restore — ${error.message}`);
    } finally {
      check.close();
    }
...
export function describe(result) {
  ...
  if (result.renumbered.length === 0) {
    return `dpm: merged cleanly — ${result.counts.inserted} rows added, `
      + `${result.counts.updated} changed, ${result.counts.deleted} removed, no number collisions`;
  }
