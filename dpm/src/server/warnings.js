...
export const isSqliteExperimental = (warning) =>
 ...
 * @param {import('node:stream').Writable} [output] Where surviving warnings go. Never stdout.
 * @returns {(warning: Error) => void} The installed listener, so a test can call it directly.
 */
export function filterWarnings(target = process, output = process.stderr) {
