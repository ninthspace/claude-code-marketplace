...
import { isAbove, parseVersion } from './node-floor.js';
import { SKEW, SOURCE } from './skew.js';
 ...
 *
 * @param {string} [moduleUrl] The URL of a module inside this plugin. Defaults to this one, which is
 *   the only value production passes; tests give it a synthetic URL to assert the arithmetic.
 * @returns {string} An absolute path — under the host's cache it is the version directory, and under
 *   a `--plugin-dir` launch it is the working tree, which is exactly the case FR1b is about.
 */
export function pluginRoot(moduleUrl = import.meta.url) {
 ...
 *
 * @param {string} root The plugin directory the server is running from.
 * @param {(path: string, options: object) => Array<{name: string, isDirectory: () => boolean}>} [read]
 *   The directory reader. A parameter so a test can count the calls and see the path.
 * @returns {string[]} Directory names, in the order the filesystem gave them.
 */
export function siblingNames(root, read = readdirSync) {
 ...
 *
 * @param {string} root The plugin directory the server is running from.
 * @param {Function} [read] The directory reader, passed through to {@link siblingNames}.
 * @returns {{state: string, running: string, newest?: string, reason?: string}}
 */
export function neighbourSkew(root, read = readdirSync) {
  ...
  try {
    ...
    return {
      ...
      state: SKEW.unknown,
      running,
      reason: `the plugin cache could not be read: ${error.message}`,
    };
  }
 ...
 *
 * @param {Function} [read] The directory reader, for tests.
 * @returns {{state: string, running: string, newest?: string, reason?: string}}
 */
export function currentSkew(read = readdirSync) {
