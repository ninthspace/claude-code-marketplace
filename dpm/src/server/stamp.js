...
import { isAbove } from './node-floor.js';
 ...
 * @returns {{present: boolean, version: string|null, reason: string|null}}
 */
export function readStamp(db) {
  let present;
  ...
  try {
    ...
    return { present: false, version: null, reason: `this database could not be read: ${error.message}` };
  }
  ...
  try {
    const version = db.prepare(`SELECT version FROM ${TABLE}`).get()?.version ?? null;
    ...
    return { present: true, version: null, reason: `the plugin stamp could not be read: ${error.message}` };
  }
}
 ...
 * @returns {string|null}
 */
export function recordedStamp(db) {
 ...
 * @param {string|null} [options.version] The running server's version. A parameter for the reason
 ...
 * @returns {{recorded: string|null, previous: string|null, written: boolean, reason?: string}}
 */
export function stampPlugin(db, { version = pluginVersion() } = {}) {
 ...
 * @param {import('node:sqlite').DatabaseSync} db
 * @param {object} [options]
 * @param {string|null} [options.version] The running server's version.
 * @returns {{state: string, running: string|null, recorded?: string, reason?: string}}
 */
export function stampSkew(db, { version = pluginVersion() } = {}) {
