 ...
 * fixed.
 */
const RANK = { [SKEW.found]: 2, [SKEW.unknown]: 1, [SKEW.none]: 0 };

/**
 * The single state describing a set of verdicts (FR5).
 *
 * @param {Array<{state: string}>} verdicts
 * @returns {string} The highest-ranking state present.
 */
export function worstState(verdicts) {
  return verdicts
    .map(({ state }) => state)
...
const SENTENCES = { [SOURCE.neighbour]: NEIGHBOUR, [SOURCE.stamp]: STAMP };
 ...
 *
 * @param {{source: string, state: string, running: string|null, newest?: string, recorded?: string,
 *   reason?: string}} skew
 * @returns {string}
 */
export function skewMessage(skew) {
 ...
 * @returns {{state: string, message: string}} Plus one key per verdict, named for its source.
 */
export function skewReport(verdicts) {
