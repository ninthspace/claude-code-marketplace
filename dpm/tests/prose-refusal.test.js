...
import { refuseBareUlids, nothingExempt } from '../src/tools/prose-refusal.js';
...
test('every column judged to hold prose is scanned, and the seam every write goes through calls it', (t) => {
  ...
  assert.deepEqual(writers, ['crud.js'],
    'every write in the server is still built in crud.js, so wiring the check there covers all of them');

  const crud = sources.find(([entry]) => entry === 'crud.js')[1];
