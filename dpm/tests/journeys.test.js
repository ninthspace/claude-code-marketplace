 ...
 * the first open is `bin/dpm-mcp.js` as a process, the refusal comes from `git commit` firing the
 * installed hook, and the import is `bin/dpm-import.js` spawned. A version of this file calling
...
import { DUMP_PATH, IMPORT_COMMAND, PUBLISH_COMMAND } from '../src/guard/index.js';
import { publish } from '../src/publish/index.js';
import { writeIgnore } from '../src/server/ignore.js';
import { MARKER_PATH, readMarker } from '../src/sync/marker.js';
...
const PUBLISH = join(ROOT, 'bin', 'dpm-publish.js');
const IMPORT = join(ROOT, 'bin', 'dpm-import.js');

// **`bin/dpm-guard.js` is deliberately absent from that list.** The guard is reached here by
