 ...
 *
 * The defect: the guard reported `differs` and ended by naming `bin/dpm-publish.js`, whatever had
 ...
 * `bin/dpm-import.js` agrees with itself forever, including after a rename, which is NFR6's failure
...
test('each verdict names the fix that belongs to it [integration]', (t) => {
  ...
  assert.match(IMPORT_COMMAND, /[/\\]bin[/\\]dpm-import\.js$/);
});
