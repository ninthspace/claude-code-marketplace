...
import { COLLECTIONS } from '../src/projection/load.js';
import { identifierOf, identifiers, pathOf, ProjectionError } from '../src/projection/naming.js';
...
test('nothing under src imports a package, so no markdown parser can be in the graph', () => {
  ...
  for (const file of javascriptFilesUnder(join(ROOT, 'src'))) {
...
test("the renderer's filesystem calls under docs/ are writes and nothing else", () => {
  const files = javascriptFilesUnder(join(ROOT, 'src', 'projection'));
  ...
  assert.deepEqual(writers.map((file) => file.replace(`${ROOT}/`, '')),
    ['src/projection/index.js'], 'writing is spread across more than one module');
});

test('nothing in the render path sorts by locale, or reads a clock', () => {
  const offenders = [];

  for (const file of javascriptFilesUnder(join(ROOT, 'src', 'projection'))) {
    const source = code(file);
