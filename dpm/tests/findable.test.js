...
import { COMMANDS, IMPORT_COMMAND, MERGE_COMMAND, PUBLISH_COMMAND } from '../src/guard/index.js';
...
test('the README names each fix, its condition, and no command a constant does not define [unit]', () => {
  ...
  // README naming `bin/dpm-whatever.js` would satisfy every containment check while sending a
  // reader to a file no constant defines and nothing asserts the existence of.
  const named = [...README.matchAll(/bin\/dpm-[a-z-]+\.js/g)].map(([path]) => path);
