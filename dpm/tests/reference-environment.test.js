...
import { REQUIRED_NODE, parseVersion } from '../src/server/node-floor.js';
import { targetVersion } from '../src/schema/migrate.js';
import {
  javascriptFilesUnder, packageManifest, sweepSourcesUnder, withoutComments,
...
test('nothing is declared to install, and there is nothing installed', () => {
  ...
  assert.equal(existsSync(join(DPM, 'node_modules')), false,
    'and no node_modules directory for a stray package to be resolved out of');
});
