...
import { currentVersion, targetVersion, versionOf } from '../src/schema/migrate.js';
...
test('the vocabulary channel contains no statement that could rewrite a term', (t) => {
  ...
  const source = readFileSync(new URL('../src/schema/seeds/index.js', import.meta.url), 'utf8')
    .replaceAll(/\/\*[\s\S]*?\*\//g, '')
    .replaceAll(/^\s*\/\/.*$/gm, '');
