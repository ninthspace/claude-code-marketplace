...
import { FTS5, hasFts5 } from '../src/db/capability.js';
import { assertNodeFloor, meetsFloor, REQUIRED_NODE } from '../src/server/node-floor.js';
...
test('process.env is read in src/ only for the sanctioned names [unit]', () => {
  ...
  assert.deepEqual(readers, ['src/db/location.js', 'src/server/read-only.js'],
...
test('the suite runs from a clean checkout with no install step [integration]', () => {
  ...
  // And the checkout this suite is running in has no `node_modules`, which is the observation
  // rather than the promise: this very run is the clean checkout the criterion is about.
  assert.equal(existsSync(join(ROOT, 'node_modules')), false,
    'this checkout has a node_modules, so the run proves nothing about a clean one');

  // The control on the manifest read: it is the real one, and it declares the runner the criterion
  // names. Without this, a typo in the path would make every assertion above hold over `undefined`.
...
test('every writer in src/ is declared, with the root it may write under [unit]', () => {
  ...
  const DECLARED = {
    'src/projection/index.js': 'the root it is handed — `docs/` in every real call',
    'src/publish/index.js': '`docs/` and `.dpm/dpm.sql` (AD11)',
    'src/rebuild/index.js': '`.dpm/`, for the staging database and the rename into place (AD16)',
    'src/server/index.js': '`.dpm/`, which it creates on the first tool call',
    'src/server/from-dump.js': 'the database path it is handed, and only to undo a failed restore',
    'src/server/ignore.js': '`.dpm/`, for the ignore file (AD15)',
    'src/sync/marker.js': '`.dpm/`, for the sync marker (AD13)',
    'src/guard/index.js': '`.dpm/`, for the sync marker on adoption only (AD13)',
  };
...
test('the static import graph from bin/dpm-mcp.js reaches no node:child_process [unit]', () => {
  const entry = join(ROOT, 'bin', 'dpm-mcp.js');
  const { complaints, examined } = auditReach(entry, 'node:child_process');

  assert.deepEqual(complaints, []);

  // **The floor, and it is the assertion that matters most here.** `bin/dpm-mcp.js` reaches the
  ...
  // **The control, taken from `src/merge/main.js` rather than from `bin/dpm-merge.js`.** Every
  ...
  const merge = auditReach(join(ROOT, 'src', 'merge', 'main.js'), 'node:child_process');

  assert.deepEqual(merge.complaints, [
    `${join(ROOT, 'src', 'merge', 'main.js')} imports node:child_process, reachable statically `
    + `from ${join(ROOT, 'src', 'merge', 'main.js')}`,
  ]);

  // And the server's own graph — the one that fills in as `src/server/` grows — is clean too. This
  // is the assertion with teeth once `bin/dpm-mcp.js`'s dynamic import is followed by a reader
  // rather than by this walker.
  assert.deepEqual(auditReach(join(ROOT, 'src', 'server', 'index.js'), 'node:child_process').complaints, []);
});
...
test('the import graph walk follows relative edges rather than stopping at the entry [unit]', () => {
  ...
  const { files, edges } = importGraph(join(ROOT, 'src', 'server', 'index.js'));

  assert.ok(files.length > 10, `the walk covered ${files.length} files, which is not this graph`);
  assert.ok(files.some((file) => file.endsWith(join('src', 'db', 'connection.js'))),
