...
import { DUMP_PATH } from '../src/guard/index.js';
import { run } from '../src/publish/main.js';

const ROOT = join(import.meta.dirname, '..');
const BIN = join(ROOT, 'bin', 'dpm-publish.js');
...
test('the executable publishes the tree rooted at the path it is given and exits zero', async (t) => {
  ...
  const checked = await runNode([join(ROOT, 'bin', 'dpm-guard.js'), repo.root], '',
...
test('the entry point refuses to run below its floor, as a process, and exits 2', async () => {
  ...
  // fixture differs from `bin/dpm-publish.js` in one constant; the message and the refusal come
...
test('each binary refuses with the exit code its fixture stands in for', () => {
  ...
  // driving it: changing `bin/dpm-publish.js`'s floor exit from 2 to 1 passed the whole suite,
  // because the only test of the refusal path runs `floor-publish.mjs` — a copy. The same hole sits
  // under `floor-entry.mjs` and `bin/dpm-mcp.js`, and has since the server shipped.
  //
  ...
  // the file — `dpm-mcp.js` has two, and they differ in neither value nor spelling.
