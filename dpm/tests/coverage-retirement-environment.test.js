...
import { REQUIRED_NODE, meetsFloor } from '../src/server/node-floor.js';
import { migrate, targetVersion } from '../src/schema/migrate.js';
import { databaseAtVersion, versionBefore } from './support/migration.js';
import { javascriptFilesUnder, packageManifest, sweepSourcesUnder, withoutComments } from './support/sources.js';
...
test('the manifest declares no dependency of either kind, and none is installed [integration]', () => {
  ...
  assert.deepEqual(manifest.devDependencies, {}, 'and no development one either');
  assert.equal(existsSync(join(DPM, 'node_modules')), false,
    'and no node_modules for a stray package to be resolved out of');
...
test('no test opens this project own planning database for writing [integration]', () => {
  ...
  const users = javascriptFilesUnder(TESTS)
