...
test('a tree whose generated files match the database passes, and says what it checked', (t) => {
  ...
  assert.match(out, /12 projected files and \.dpm\/dpm\.sql match the database/);
  ...
  assert.equal(result.checked.files, 12);
});
...
test('a hand-edited generated file fails the guard, naming the file', (t) => {
  const repo = repository(t);
  const file = generated(repo.root, 'docs/specifications/1-spec-persistence.md');
  ...
  assert.equal(code, 1);
  assert.match(err, /docs\/specifications\/1-spec-persistence\.md — differs/);
  assert.match(err, /Nothing was written/);
});

test('must NOT — a hand-edit is silently overwritten with no diagnostic', (t) => {
  const repo = repository(t);
  const file = generated(repo.root, 'docs/epics/1-01-epic-projection.md');
  ...
  assert.deepEqual(readdirSync(join(repo.root, 'docs', 'epics')).sort(),
    ['1-01-coverage_matrix-projection.md', '1-01-epic-projection.md']);
});
...
test('an edit that changes bytes and not meaning still fails', (t) => {
  ...
  assert.match(err, /1-spec-persistence\.md — differs/);
});
...
test('reordering two metadata lines fails, though every field survives', (t) => {
  ...
  assert.match(err, /1-01-epic-projection\.md — differs/);
});

test('the guard reads generated files whole and compares them to regenerated bytes', () => {
  const files = javascriptFilesUnder(join(ROOT, 'src/guard'));
...
test('a write since the last regeneration leaves the dump stale, and the guard names it', (t) => {
  ...
  repo.call.dpm_create_spec({ slug: 'search', title: 'Full-text search' });
  ...
  assert.match(err, /docs\/specifications\/2-spec-search\.md — is not on disk/);
...
test('a generated file no document produces is reported, and a hand-kept one is not', (t) => {
  ...
  const orphan = join(repo.root, 'docs', 'retros', '9-retro-gone.md');
  ...
  assert.equal(code, 1);
  assert.match(err, new RegExp(`docs/retros/9-retro-gone\\.md — ${DIVERGENCE.orphaned}`));
...
test('the executable exits zero on a clean tree and non-zero on a diverged one', async (t) => {
  ...
  const file = generated(repo.root, 'docs/runbooks/1-runbook-restore.md');
  ...
  assert.match(diverged.stderr, /1-runbook-restore\.md — differs/);
...
test('the shipped pre-commit hook runs the guard and repairs nothing', () => {
  ...
  assert.match(hook, /bin\/dpm-guard\.js/);
