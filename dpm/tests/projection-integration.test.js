 ...
 * is not ceremony. Story 3 exercised `run()` and `bin/dpm-guard.js` directly and passed every time
 * against a hook that had never once been reached by git — `dirname "$0"` on the installed symlink
 * is `.git/hooks`, so the hook looked for the executable at `.git/bin/dpm-guard.js`. Nothing below
...
import { DUMP_PATH, guard } from '../src/guard/index.js';
import { run as runGuard } from '../src/guard/main.js';
import { run as runMerge } from '../src/merge/main.js';
import { identifiers, ProjectionError } from '../src/projection/naming.js';
