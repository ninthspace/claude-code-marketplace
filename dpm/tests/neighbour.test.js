...
import { currentSkew, neighbourSkew, pluginRoot, siblingNames } from '../src/server/neighbour.js';
import { SKEW, SOURCE, skewMessage } from '../src/server/skew.js';
import { pluginCache as cacheOf } from './support/plugin-cache.js';
import { filesUnder, javascriptFilesUnder, packageManifest, withoutComments } from './support/sources.js';

const pluginCache = (t, versions, options = {}) =>
  cacheOf(t, versions, { prefix: 'dpm-neighbour-', ...options });

const RESOLVER = fileURLToPath(new URL('../src/server/neighbour.js', import.meta.url));
 ...
 */
const COMPOSER = fileURLToPath(new URL('../src/server/skew.js', import.meta.url));
...
test('resolving from the module URL names the directory the module was loaded from [unit]', () => {
  // `<root>/src/server/neighbour.js` — built by walking up from the source rather than written as a
  ...
  assert.equal(
    pluginRoot(new URL('file:///somewhere/0.4.0/src/server/neighbour.js').href),
    '/somewhere/0.4.0',
...
test('the skew sentence is composed in exactly one place [unit]', () => {
  ...
  const composing = javascriptFilesUnder(fileURLToPath(new URL('../src', import.meta.url)))
