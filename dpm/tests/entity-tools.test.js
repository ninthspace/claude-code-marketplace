...
import { documentTools } from '../src/tools/spine/document.js';
import { DETAIL } from '../src/tools/spine/detail.js';
...
function roots(call) {
  ...
  const epic = call.dpm_create_epic({ parent_id: spec.id, slug: 'parity', title: 'Parity and search' });
...
test('an observation written against a story keeps its story_id when a retro gathers it', (t) => {
  ...
  const observed = call.dpm_create_observation({
    story_id: story.id,
  ...
  const retro = call.dpm_create_retro({ parent_id: epic.id, slug: 'parity', title: 'Parity retro' });
  const gathered = call.dpm_update_observation({
    id: observed.id,
    retro_id: retro.id,
    position: 2,
  ...
  const byOrigin = call.dpm_read_observation({ id: observed.id, include_body: true });
  ...
  const direct = call.dpm_create_observation({
...
test('the mutation: an update that wrote every column would clear the origin', (t) => {
  ...
  const retro = call.dpm_create_retro({ parent_id: epic.id, slug: 'r', title: 'R' });

  // What `dpm_update_observation` would do if it wrote its whole field list rather than the
  ...
  assert.equal(call.dpm_read_observation({ id: observed.id }).story_id, null);
  ...
  const second = call.dpm_create_observation({ story_id: story.id, text: 'origin', position: 1 });

  call.dpm_update_observation({ id: second.id, retro_id: retro.id, position: 4 });

  assert.equal(call.dpm_read_observation({ id: second.id }).story_id, story.id);
});
...
test('a review and a retro are root-numbered and still record what they were about', (t) => {
  ...
  const onSpec = call.dpm_create_review({ parent_id: spec.id, slug: 'spec', title: 'Spec review' });
  ...
  const wrong = refused(() => call.dpm_create_review({
...
test('the mutation: a review built as though root numbering meant no parent', (t) => {
  ...
  assert.equal(call.dpm_create_review({
...
test('a structured kind writes its document and its detail row together', (t) => {
  ...
  assert.equal(call.dpm_read_adr({ id: adr.id }).decision_status, 'proposed');

  // An update reaching both tables, in one call and one transaction.
  const accepted = call.dpm_update_adr({
  ...
  const quick = call.dpm_create_quick({ slug: 'sweep', title: 'A quick sweep' });
...
test('the mutation: a detail row that fails leaves no document row behind', (t) => {
  ...
  const omitted = refused(() => call.dpm_create_adr({
  ...
  const dangling = refused(() => call.dpm_create_review({
  ...
  const story = call.dpm_create_story({
    epic_id: call.dpm_create_epic({ parent_id: spec.id, slug: 'e', title: 'E' }).id,
    number: 1, title: 'S', position: 0,
  });
  const scoped = call.dpm_create_review({
...
test('a milestone join across two specs is refused, and one within a spec is not', (t) => {
  ...
  assert.equal(call.dpm_create_document_milestone({
    document_id: epic.id, milestone_id: milestone.id,
  }).milestone_id, milestone.id);

  // The spec itself delivering its own milestone — the walk's zero-step case.
  assert.ok(call.dpm_create_document_milestone({
    document_id: spec.id, milestone_id: milestone.id,
  }));

  const other = call.dpm_create_spec({ slug: 'other', title: 'Another spec' });
  const otherEpic = call.dpm_create_epic({
  ...
  const across = refused(() => call.dpm_create_document_milestone({
...
test('every remaining type creates and reads back through its own tools', (t) => {
  ...
  const retro = call.dpm_create_retro({ parent_id: epic.id, slug: 'r', title: 'Retro' });
  const audit = call.dpm_create_audit({ slug: 'a', title: 'Audit' });
  const quick = call.dpm_create_quick({ slug: 'q', title: 'Quick' });
  const library = call.dpm_create_library({ slug: 'l', title: 'Library', doc_type: 'architecture' });
  const adr = call.dpm_create_adr({ parent_id: spec.id, slug: 'a', title: 'ADR', decision: 'd' });
  const matrix = call.dpm_create_coverage_matrix({ parent_id: epic.id, slug: 'm', title: 'Matrix' });
  ...
  for (const kind of ['problem_brief', 'product_brief', 'discussion', 'runbook']) {
    const made = call[`dpm_create_${kind}`]({ slug: kind, title: kind });

    assert.equal(call[`dpm_read_${kind}`]({ id: made.id }).kind, kind);
  ...
  const option = call.dpm_create_adr_option({ adr_id: adr.id, name: 'Do nothing', position: 0 });
  const tradeoff = call.dpm_create_adr_option_tradeoff({
    option_id: option.id, axis: 'reversibility', assessment: 'Total.',
  });
  const agent = call.dpm_create_review_agent({ document_id: review.id, agent: 'architect' });
  const criterion = call.dpm_create_quick_criterion({
    quick_id: quick.id, text: 'it holds', position: 0,
  });
  const scope = call.dpm_create_library_scope({ document_id: library.id, scope: 'do' });
  ...
  const auditFinding = call.dpm_create_audit_finding({
  ...
  const application = call.dpm_create_retro_application({
  ...
  const published = call.dpm_create_artifact_document({
  ...
  const alsoBy = call.dpm_create_coverage_story({ coverage_id: coverage.id, story_id: story.id });
  ...
  assert.equal(call.dpm_read_adr_option({ id: option.id }).name, 'Do nothing');
  assert.equal(call.dpm_read_adr_option_tradeoff({
    option_id: option.id, axis: 'reversibility', include_body: true,
  }).assessment, 'Total.');
  assert.equal(call.dpm_read_review_agent({ document_id: review.id, agent: 'architect' }).agent,
    agent.agent);
  assert.equal(call.dpm_read_quick_criterion({ id: criterion.id }).met, null);
  assert.equal(call.dpm_read_library_scope({ document_id: library.id, scope: 'do' }).scope,
    scope.scope);
  assert.equal(call.dpm_read_document_section({ id: section.id }).heading, 'Data Model');
  assert.equal(call.dpm_read_finding({ id: finding.id }).status, 'open');
  assert.equal(call.dpm_read_audit_finding({ id: auditFinding.id }).line, 1);
  assert.equal(call.dpm_read_retro_application({ id: application.id }).disposition, 'applied');
  assert.equal(call.dpm_read_artifact({ id: artifact.id }).title, 'A');
  assert.equal(call.dpm_read_artifact_document({
    artifact_id: artifact.id, document_id: spec.id,
  }).document_id, published.document_id);
  assert.equal(call.dpm_read_coverage_story({
  ...
  const bare = call.dpm_create_retro_application({
  ...
  assert.equal(call.dpm_update_adr_option({ id: option.id, chosen: true }).chosen, 1);
...
test('a read or update tool named for a kind refuses a document of another kind', (t) => {
  ...
  const audit = refused(() => call.dpm_read_audit({ id: spec.id }));
  ...
  const before = call.dpm_read_spec({ id: spec.id }).updated_at;
  const wrong = refused(() => call.dpm_update_retro({ id: epic.id, title: 'Not a retro' }));

  assert.match(wrong.message, /is a epic, not a retro/);
  assert.equal(call.dpm_read_spec({ id: spec.id }).updated_at, before);
  assert.equal(call.dpm_read_epic({ id: epic.id }).title, 'Parity and search');
});
