...
3. Otherwise `mcp__dpm__list_spec` — offer the results with `AskUserQuestion`, showing each title.
...
`mcp__dpm__list_adr` with the source spec as `parent_id`. Where decisions exist, summarise them and
...
Read the spec and its parts: `mcp__dpm__read_spec`, `mcp__dpm__list_requirement` with a `limit`
above the spec's requirement count, and `mcp__dpm__list_document_section` for the scope boundary and
...
On approval, each agreed epic is one `mcp__dpm__create_epic` call, with the spec as `parent_id`.
...
Then, for each epic, `mcp__dpm__create_coverage_matrix` with that epic as `parent_id`. It carries no
rows of its own — Step 3d writes those — and it exists so the matrix has somewhere to render.

Where one epic cannot start until another finishes, record it with `mcp__dpm__create_dependency`:
...
Each agreed story is one `mcp__dpm__create_story` call under its epic, taking `number` (ordinal
...
Where a story cannot start until another finishes, record it with `mcp__dpm__create_dependency`:
...
Each criterion is `mcp__dpm__create_story_criterion` under its story, with `text` and `position`.
...
`mcp__dpm__list_acceptance_criterion` and give every criterion whose `polarity` is `must_not` a
...
Each criterion's approach is `mcp__dpm__create_story_criterion_approach`, naming the criterion and
the `tag`. A criterion verified two ways carries two of them. `mcp__dpm__list_test_approach` returns
...
requirement's criteria and their `mcp__dpm__list_criterion_approach` rows, and apply the same tags.
...
`mcp__dpm__create_task` call under its story, taking `number` (ordinal within the story), `title` in
...
`mcp__dpm__create_coverage` call taking:
...
declares it, add `mcp__dpm__create_coverage_story` naming that story.
...
`mcp__dpm__update_story_criterion` before moving on.
...
from `mcp__dpm__list_requirement`, call `mcp__dpm__list_coverage` on it. A requirement with no
...
**Read the tree back rather than repeating what was sent.** `mcp__dpm__list_story` per epic,
`mcp__dpm__list_task` and `mcp__dpm__list_story_criterion` per story. A value that never reached a
...
`mcp__dpm__create_document_section` on the epic, naming the clause and the story it would have gone
...
recorded rather than attached — with `mcp__dpm__create_document_section`, each with its heading and
