...
INSERT INTO "coverage" ("id", "requirement_id", "spec_fragment", "story_criterion_id", "position", "verified_at", "binding_hash") VALUES ('01M05QBY74E5ZT3GXPBPVMAK12', '01M05JW91578GQ942N25MEHN7Y', '`r` refreshes, `R` clears the cache, `a` registers a project, `x` unregisters the highlighted one', '01M05P6GJQNEJ96Q0Q88MBNC98', 0, NULL, NULL);
INSERT INTO "coverage" ("id", "requirement_id", "spec_fragment", "story_criterion_id", "position", "verified_at", "binding_hash") VALUES ('01M05QC077E8Y2X9G9X1Q4GWJJ', '01M05JW91578GQ942N25MEHN7Y', 'A capability the DPM board reaches only through the command palette today gains its CPM key.', '01M05P6HY7K4WSK6SX299EED7H', 1, NULL, NULL);
INSERT INTO "coverage" ("id", "requirement_id", "spec_fragment", "story_criterion_id", "position", "verified_at", "binding_hash") VALUES ('01M05QC1KD87QXJHJJH4940897', '01M05JW91578GQ942N25MEHN7Y', 'Every key the CPM board binds carries the same meaning on the DPM board', '01M05P6KA007DFB84N2463DKXR', 2, NULL, NULL);
...
INSERT INTO "session" ("id", "skill", "phase", "state", "superseded_by", "created_at", "updated_at") VALUES ('d5d4f15e-4995-4cd2-84cc-1359f0c4bf1e-do-e1', 'dpm:do', 'Epic 1, story 3 — next ready story', '{"test_command":"uv run pytest, from dpm/tools/board","framework":"none detected (PEP 723 single-file scripts, pytest 8 + pytest-asyncio, asyncio_mode=auto)","retro_gate":{"applied":3,"deferred":4},"stories":{"01M05NTFP3SP5G00T656J93QFX":{"title":"A fixture project holding every state","outcome":"complete","refactor":"ran — consolidation was `statuses()` in fixture_database.py, replacing a literal status set in test_client.py; suite green","notes":"fixture already existed; extended. Fixed an unrelated baseline failure (test_states.py spec path moved to docs/cpm/). Two tests transcribed the fixture''s shape and were re-derived."},"01M05NTGKBR1C512G581KVZH52":{"title":"The environment this work rests on","outcome":"complete","refactor":"ran, no change — the story touched one test module and its new helpers (canonical, requirement_name, provisioned, top_level_imports) have a single consumer each; nothing to merge","notes":"5 of 6 criteria verified; the 6th is target-only (host Python floor) and is recorded as a section on the epic. board.py already imported rich, so the import-surface check resolves the transitive closure of the inline block rather than its literal list."}}}', NULL, '2026-08-16T17:21:26.993Z', '2026-08-16T17:41:54.295Z');
INSERT INTO "session" ("id", "skill", "phase", "state", "superseded_by", "created_at", "updated_at") VALUES ('d5d4f15e-4995-4cd2-84cc-1359f0c4bf1e-ralph', 'dpm:ralph', 'Iteration 2 — epic 1', '{"harness_session_id":"d5d4f15e-4995-4cd2-84cc-1359f0c4bf1e","note":"The harness session id already keys the dpm:spec/dpm:epics row for this conversation, so this run''s row is keyed on a derived id. The state file carries the harness id — that is what the stop hook matches.","mode":"epic","argument":"epics 1-4","epics":["01M05NNYYBX6XYRMND1CN4M4TY","01M05NNZQE4VVEWMCXGMZ72YJ5","01M05NP14C3GCH372FM6NN2T5S","01M05NP15VPTJKBNK0B2HQ4XCZ"],"max_iterations":50,"completion_promise":"ALL_EPICS_COMPLETE","hook":"ralph-loop@ninthspace-ralph 1.2.1, the only enabled Stop hook; ralph-wiggum@claude-code-plugins and ralph-loop@claude-plugins-official are installed and disabled","probe":"dpm 0.5.0 ships no hooks/lib/ralph-hook-probe.sh, so the skill''s probe could not be run; substituted the plugin''s own tests/test-fail-closed.sh, which passed 10/10 including a turn ending on a tool call keeping the loop alive","plan_flags_cleared":["01M05PHNRSGHHF8844AHMJ2E7M","01M05QSAPA8MP912VBFV9M9GEX"],"gitignore":"added .claude/ralph-loop.local.md","launch_commit":"b18a6c1","entry_numbering":"entries carry the stop hook''s iteration counter, so iteration 1 (the arming turn, which did no work) has no entry","iterations":[{"iteration":2,"counts":{"stories_complete":0,"stories_total":16,"coverage_verified":0,"coverage_total":47},"commit":"b18a6c1","tree":"3b80bd9bf805"}]}', NULL, '2026-08-16T17:14:05.037Z', '2026-08-16T17:21:12.341Z');
...
INSERT INTO "story" ("id", "epic_id", "epic_kind", "number", "title", "status", "status_note", "position", "plan") VALUES ('01M05P6A9HYV9QSZJNAP2NB3CY', '01M05NNZQE4VVEWMCXGMZ72YJ5', 'epic', 1, 'CPM''s key meanings restored', 'pending', NULL, 0, 0);
