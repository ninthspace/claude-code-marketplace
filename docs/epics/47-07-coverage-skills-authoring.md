...
| 10 | FR24 | Observation categories, finding categories, audit dimensions, severities, test approaches and **agent personas** are rows referenced by foreign key | A review run writes `review` with its `scope` and `scope_story_id`, `review_agent` rows referencing `agent` rows rather than carrying persona names as text, and `finding` rows with severity and category as taxonomy references | Story 4 | `[feature]` | |
