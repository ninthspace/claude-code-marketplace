...
| 12 | NFR4 | The same database state produces the same `.sql` bytes on any machine, on any run — ordered rows, no timestamps, no locale dependence. | A dump taken before and after a no-op read produces identical bytes, so reading does not perturb dump order | Story 4 | `[integration]` | |
...
was not a cascaded change: the criterion was already on Story 4 and had no row, found by a
