...
| 14 | NFR2 | renders as something and does not take the board down | control — With the guard removed, a render that raises takes the board down — so the criterion above cannot pass against a panel that was never protecting itself. No markdown source has been found that makes the renderer raise, so the control drives the failure through the renderer rather than through the input. | Story 4 | `[unit]` | ✓ |
