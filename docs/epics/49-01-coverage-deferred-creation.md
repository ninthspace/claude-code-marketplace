...
| 19 | NFR3 | "An existing `.dpm/dpm.db` opens and serves exactly as today: no rebuild, no prompt, no migration beyond what `migrate()` already does." | "An existing database with rows has an identical *content* hash — the `dump()` text — before and after a read-only lazy session, applies no migration, re-seeds no vocabulary, and answers the same reads" | Story 5 | `[integration]` | ✓ |
